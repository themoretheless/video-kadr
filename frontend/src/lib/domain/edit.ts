...
export function buildEditPayload(
  ...
  const supportsSegments = supportsTimelineFormat(edit.format)
  if (edit.timelineEnabled && supportsSegments) {
    payload.segments = activeTimelineSegments(edit, video.duration).map(({ start, end }) => ({
      start,
      end,
    }))
  } else {
  ...
  if (edit.chromaKeyEnabled) {
    payload.chromaKey = {
      ...
      similarity: sanitizeChromaSimilarity(edit.chromaKeySimilarity),
      blend: sanitizeChromaUnit(edit.chromaKeyBlend, EDIT_DEFAULTS.chromaKeyBlend),
      spillSuppression: sanitizeChromaUnit(edit.chromaKeySpill, EDIT_DEFAULTS.chromaKeySpill),
    }
  }
...
export function sanitizeChromaKeyColor(value: unknown): string {
  if (typeof value !== 'string') return EDIT_DEFAULTS.chromaKeyColor
  const match = CHROMA_HEX.exec(value.trim())
  return match ? `#${match[1]!.toLowerCase()}` : EDIT_DEFAULTS.chromaKeyColor
}

export function sanitizeChromaSimilarity(value: unknown): number {
  return Math.max(0.00001, Math.min(1, finiteOr(value, EDIT_DEFAULTS.chromaKeySimilarity)))
}
...
export function sanitizeEditState(value: unknown, base: EditState = defaultEdit()): EditState {
  ...
  const cut = isRecord(source.cut)
    ? { ...base.cut, ...source.cut } as EditState['cut']
    : { ...base.cut }
  const baseTimelineEnd = base.timelineSegments.reduce(
    (maximum, segment) => Math.max(maximum, finiteOr(segment.end, 0)),
    0,
  )
  const baseDuration = Math.max(finiteOr(base.trimEnd, 0), baseTimelineEnd)
  const timelineDuration =
    baseDuration > 0 ? baseDuration : Math.max(0, finiteOr(merged.trimEnd, 0))
  const hasStoredTimeline = Object.hasOwn(source, 'timelineSegments')
  const storedTimeline = sanitizeTimelineSegments(
    hasStoredTimeline ? source.timelineSegments : base.timelineSegments,
    timelineDuration,
  )
  const migratesLegacyCut =
    !Object.hasOwn(source, 'timelineEnabled') &&
    !hasStoredTimeline &&
    source.cutEnabled === true
  const timelineEnabled =
    typeof source.timelineEnabled === 'boolean'
      ? source.timelineEnabled
      : hasStoredTimeline
        ? storedTimeline.length > 0
        : migratesLegacyCut
          ? true
          : base.timelineEnabled
  const timelineSegments =
    timelineEnabled && !storedTimeline.length
      ? timelineSegmentsFromLegacy(
          {
            trimStart: merged.trimStart,
            trimEnd: merged.trimEnd,
            cutEnabled: merged.cutEnabled,
            cut,
          },
          timelineDuration,
        )
  ...
  return {
    ...
    censor: isRecord(source.censor)
      ...
      : { ...base.censor },
    chromaKeyEnabled:
      typeof source.chromaKeyEnabled === 'boolean'
        ? source.chromaKeyEnabled
        : base.chromaKeyEnabled,
    chromaKeyColor: sanitizeChromaKeyColor(source.chromaKeyColor ?? base.chromaKeyColor),
    chromaKeySimilarity: sanitizeChromaSimilarity(
      source.chromaKeySimilarity ?? base.chromaKeySimilarity,
    ),
    chromaKeyBlend: sanitizeChromaUnit(
      source.chromaKeyBlend ?? base.chromaKeyBlend,
      base.chromaKeyBlend,
    ),
    chromaKeySpill: sanitizeChromaUnit(
...
export function resetColorAdjustments(edit: EditState): void {
  ...
  edit.chromaKeyBlend = EDIT_DEFAULTS.chromaKeyBlend
  edit.chromaKeySpill = EDIT_DEFAULTS.chromaKeySpill
