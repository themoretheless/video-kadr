...
- Загруженный через «Импорт локального файла» файл на данный момент **не
  проверяется по содержимому** (только по расширению + тому, что ffprobe
  извлёк хоть какие-то метаданные) - подтверждён рабочий эксплойт (stored XSS
  через файл-полиглот, [docs/audit.md](docs/audit.md) №202). Не загружай файлы
  из недоверенных источников до фикса. Импорт по ссылке также может быть
  уведён редиректом на приватный адрес в обход SSRF-проверки ([docs/audit.md](docs/audit.md) №201).
