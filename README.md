...
Видеоредактор в браузере. Статическая Pages-версия обрабатывает локальные файлы
через `ffmpeg.wasm`; полноценный режим использует Rust (Axum), `yt-dlp` и нативный
`ffmpeg` для импорта по ссылкам и серверного экспорта. Фронтенд — Vue 3 + Vite.
...
## Что умеет редактор (расширенный режим)

Всё ниже доступно только в серверном режиме (нужен нативный `ffmpeg`); в
Pages-версии на `ffmpeg.wasm` эти панели недоступны. Каждая возможность
проверяется реальным рендером в `backend/tests/parity_render.rs`.

- **Таймлайн из нескольких клипов.** Панель «Таймлайн»: разрезание по плейхеду
  (`S`), ripple-удаление, перетаскивание для смены порядка, растягивание границ,
  привязка к плейхеду/краям/маркерам, маркеры (`M`), превью-кадры и звуковая
  дорожка. Клипы задают точки входа/выхода и скорость по каждому куску.
- **Переходы.** 17 типов (`fade`, `wipeleft/right/up/down`, `slide*`,
  `circleopen/close`, `dissolve`, `pixelize`, `radial`, `smoothleft/right`,
  `zoomin`) длительностью 0.05–3 с. Видео сшивается через `xfade`, звук через
  `acrossfade`; длительность экспорта учитывает перекрытие.
- **Наложения (PiP, водяной знак, хромакей).** До 32 слоёв: картинка или видео,
  позиция и размер в долях кадра, поворот, прозрачность, окно показа,
  fade in/out, хромакей с подбором цвета пипеткой. У видео-слоя звук попадает
  в общий микс.
- **Титры и субтитры.** До 32 титров: шрифт из ассетов, размер, цвет,
  выравнивание, подложка, обводка, тень, анимации (`fade`, `slide-up`,
  `typewriter`, `pop`). Субтитры `.srt`/`.vtt` с редактором реплик и вшиванием
  в картинку. Требует `ffmpeg` с `drawtext` (freetype) и `subtitles` (libass).
- **Микс звука.** До 8 дополнительных дорожек (музыка, закадр, эффекты) с
  раскладкой на таймлайне, зацикливанием, fade in/out и авто-приглушением под
  голос (`sidechaincompress`). Обработка голоса: шумодав, дереверб, де-эссер,
  гейт, компрессор, лимитер, ФВЧ/ФНЧ, огибающая громкости по ключевым кадрам,
  выбор битрейта 64–320 кбит/с.
- **Анимация кадра и рампы скорости.** Ken Burns: ключевые кадры зума, панорамы
  и поворота с редактором кривой и предпросмотром рамок «старт»/«финиш». Рампы
  скорости 0.25×–4× с сохранением синхронности звука.
- **360 и экшн-камеры.** Перекадрирование сферического видео (`v360`):
  проекции входа `equirect`/`fisheye`/`dfisheye`, выхода
  `flat`/`equirect`/`fisheye`/`stereographic`/`pannini`, анимируемые
  yaw/pitch/roll/FOV, блокировка горизонта, WebGL-предпросмотр с перетаскиванием.
  Стабилизация: быстрая (`deshake`) и точная в два прохода (`vidstab`, требует
  `ffmpeg` с `libvidstab`). Коррекция дисторсии объектива (k1/k2).
- **Расширенный цвет (Resolve-lite).** Температура и оттенок, экспозиция,
  света/тени, колёса lift/gamma/gain, HSL по шести диапазонам, скопы
  (гистограмма, waveform, RGB parade, вектороскоп) и сравнение до/после шторкой.
  Диапазон `orange` в HSL не поддерживается: у `huesaturation` его нет.
- **Ассеты.** `POST /api/assets` принимает картинки, видео, звук, шрифты и
  субтитры; тип определяется по сигнатуре файла, а не по имени или заголовку.
- **Пакетный экспорт.** Пресеты соотношений (9:16, 1:1, 4:5, 16:9, 2.39:1) и
  площадок (Reels/Shorts/TikTok/YouTube) с очередью задач.

Если у локального `ffmpeg` нет нужного фильтра, запрос отклоняется сразу с
понятным сообщением, а не падает внутри процесса.
...
POST /api/assets            multipart file + kind        -> AssetRecord
GET  /api/assets                                           -> { assets: [ AssetRecord ] }
GET  /api/assets/:id                                       -> AssetRecord | 404
DELETE /api/assets/:id                                     -> 204 | 404
...
непосредственно из статической страницы из-за CORS и отсутствия `yt-dlp` в
браузере. Обычный локальный запуск без `VITE_PROCESSING_MODE=browser` продолжает
использовать Rust backend.
