...
- [ ] **`library.save()` глотает ошибки записи** - на полном диске `add()` вернёт успех, запись потеряна. Сделать `save()->Result`, логировать, пробрасывать. `library.rs:80-88`
- [ ] **`library.remove()` удаляет файл до `save()`** - при сбое save запись воскреснет, указывая на удалённый файл. Сначала `save()`, потом удалять файл. `library.rs:125-136`
- [ ] **`from_result` пустой id схлопывает медиа** - `unwrap_or_default()` даёт `id=""`, dedup по id сливает разные записи. Возвращать `Option`, отбраковывать пустые id/filename/url. `library.rs:33-46`
