...
18. ☐ Upload минует semaphore → P0 backlog/P2-9. *(перепроверено 1 июля 2026, не тронуто)*
...
33. ◐ DNS SSRF bypass → P0-3. *(DNS-резолвинг закрыт; редирект `yt-dlp` - новый P0, audit.md №201)*
...
178. No redirect policy → downloader adapter constraints. *(конкретизировано audit.md №201, подтверждённый обход, новый P0)*
...
182. Upload magic bytes missing → media probe before publish. *(конкретизировано audit.md №202, подтверждённый PoC stored XSS, новый P0)*
...
- [ ] 🟠 **223.** (проблема) upload_handler не создаёт job и не проходит через jobs_semaphore, обходя лимит одновременных операций -> Провести upload через тот же jobs_semaphore.acquire перед записью на диск, либо явно задокументировать, почему upload намеренно вне лимита, и добавить отдельный лимит параллельных upload. `backend/src/handlers/mod.rs`
- [ ] 🟠 **225.** (баг) upload_handler не ограничивает время probe_video и не даёт отмену, в отличие от import/edit -> Обернуть probe_video в upload_handler в tokio::time::timeout(job_timeout(), ...) и вернуть 500/504 при истечении. `backend/src/handlers/mod.rs`
- [ ] 🟠 **226.** (проблема) normalize_edit_request и валидаторы диапазонов — доменная логика в HTTP-слое -> Перенести эти функции в model.rs или отдельный модуль domain/edit_validation.rs, откуда их будет импортировать edit_handler. `backend/src/handlers/mod.rs`
...
- [ ] 🟡 **235.** (улучшение) project_upsert_handler не проверяет, что video["id"] совпадает с videoId тела запроса -> После разбора video проверить video["id"].as_str() == Some(video_id) и вернуть 400 при расхождении. `backend/src/handlers/projects.rs`
- [ ] 🟡 **236.** (дизайн) Ошибки в projects.rs смешивают русский текст с сырыми деталями SQLx в INTERNAL_SERVER_ERROR -> Логировать e через tracing::error! на сервере и возвращать клиенту фиксированное русскоязычное сообщение 'внутренняя ошибка' без деталей SQLx. `backend/src/handlers/projects.rs`
- [ ] 🟡 **242.** (улучшение) health_handler жёстко завязан на ровно два внешних инструмента -> Хранить инструменты как Vec<(name, ToolStatus)> в ToolInfo и сериализовать их в цикле, чтобы health_handler не менялся при добавлении нового инструмента. `backend/src/handlers/health.rs`
...
- [ ] 🟡 **231.** (проблема) edit_handler мутирует входящий EditRequest прямо в воркере вместо получения провалидированного типа -> Изменить normalize_edit_request на fn(EditRequest, ...) -> anyhow::Result<NormalizedEditRequest>, чтобы невалидированный EditRequest не мог случайно использоваться дальше по ошибке. `backend/src/handlers/mod.rs`
...
- [ ] 🟡 **233.** (проблема) project_upsert_handler вручную парсит сырой serde_json::Value вместо типизированного тела запроса -> Ввести #[derive(Deserialize)] struct ProjectUpsertRequest { video_id: String, video: Value, edit: Value, name: Option<String> } и десериализовать через Json<ProjectUpsertRequest>. `backend/src/handlers/projects.rs`
- [ ] 🟡 **234.** (проблема) ensure_project_json_size сериализует video/edit отдельно от последующей записи в БД, дублируя сериализацию -> Сериализовать video/edit один раз в project_upsert_handler, проверить длину байт и передать уже сериализованные строки в upsert_project, чтобы БД не сериализовала повторно. `backend/src/handlers/projects.rs`
- [ ] 🟡 **237.** (проблема) project_get_handler и project_by_video_handler возвращают голый StatusCode без текста ошибки, в отличие от upsert/list -> Привести все четыре project-хендлера к единому Result<_, (StatusCode, String)> с логированием причины на сервере. `backend/src/handlers/projects.rs`
- [ ] 🟡 **238.** (проблема) Паттерн 'Ok(Some)/Ok(None)/Err -> StatusCode' повторяется идентично в нескольких хендлерах -> Добавить extension-trait для Result<Option<T>, sqlx::Error> с методом .into_response_or_404(), переиспользуемый во всех подобных хендлерах. `backend/src/handlers/projects.rs`
...
- [ ] 🟡 **241.** (проблема) health_handler не проверяет живость хранилища/БД, только статически захваченные при старте флаги ffmpeg/ytdlp -> Добавить лёгкую проверку доступности storage-директорий и SQLite-пула (например, SELECT 1) прямо в health_handler с коротким таймаутом. `backend/src/handlers/health.rs`
- [ ] 🟡 **243.** (проблема) health_handler не различает, какой из статусов 'ok'/'degraded' к чему относится при отсутствии обоих инструментов -> Добавить массив missingTools в ответ, перечисляющий конкретные недоступные инструменты, чтобы не заставлять клиента сверять два булевых поля. `backend/src/handlers/health.rs`
...
- [ ] 🟡 **260.** (проблема) project_upsert_handler инлайн реализует fallback-цепочку для имени проекта вместо доменной функции -> Вынести резолюцию имени в отдельную чистую функцию resolve_project_name(body, video) -> String, тестируемую независимо от HTTP-слоя. `backend/src/handlers/projects.rs`
- [ ] 🟡 **261.** (проблема) project_upsert_handler не ограничивает длину поля name -> Добавить проверку name.len() <= разумного предела (например 200 байт) и обрезать/отклонить при превышении, аналогично ensure_project_json_size для video/edit. `backend/src/handlers/projects.rs`
...
- [ ] 🟡 **264.** (баг) sanitize_ext не проверяет конфликт очищенного расширения с зарезервированными именами -> Явно исключить зарезервированные суффиксы (part, tmp, info.json-подобные) из допустимых значений ext в sanitize_ext. `backend/src/handlers/mod.rs`
...
- [ ] 🟡 **268.** (проблема) Хендлеры projects.rs возвращают внутреннюю структуру Project из db.rs напрямую как HTTP DTO -> Ввести отдельный ProjectResponse в handlers/projects.rs с явным From<Project>, чтобы изменения схемы БД не автоматически меняли HTTP-ответ. `backend/src/handlers/projects.rs`
...
- [ ] 🟡 **278.** (проблема) jobs_semaphore — публичное поле AppState, а не метод, что нарушает инкапсуляцию по сравнению с приватными jobs/cancels/render_locks. -> Сделать поле приватным и добавить метод acquire_job_permit(&self) -> impl Future<...>, инкапсулирующий получение permit, единообразно с остальными приватными коллекциями. `backend/src/state.rs`
...
- [ ] 🟡 **288.** (баг) check_tool не имеет таймаута — при зависшем бинарнике (--version) health-check на старте может повиснуть навсегда. -> Обернуть .output().await в tokio::time::timeout(Duration::from_secs(5), ...) и трактовать истечение как отсутствие инструмента. `backend/src/tools/mod.rs`
- [ ] 🟡 **291.** (баг) validate_url вызывается один раз перед стартом импорта, но yt-dlp сам делает HTTP-редиректы — TOCTOU между валидацией исходного URL и фактическим доступом yt-dlp по редиректной цепочке. -> Либо запускать yt-dlp с флагом ограничения редиректов на внешние хосты, либо повторно валидировать финальный resolved URL, который yt-dlp фактически использовал (если он его логирует). `backend/src/handlers/mod.rs`
...
- [ ] 🟡 **298.** (проблема) EditRequest не валидирует диапазоны большинства числовых полей на уровне модели — rotate, quality, censor и другие принимают любые значения из JSON без ограничений. -> Либо добавить #[serde(deserialize_with = ...)] валидаторы на самые опасные поля (rotate, speed, quality), либо явно задокументировать в модели, что EditRequest — сырой DTO и вся валидация намеренно вынесена в normalize_edit_request. `backend/src/model.rs`
...
- [ ] 🟡 **311.** (баг) cache_key в edit_handler вычисляется до normalize_edit_request, поэтому эквивалентные после клэмпинга запросы (например rotate=450 и rotate=90) не разделяют один и тот же render-кэш. -> Вызывать normalize_edit_request до вычисления render_cache_key (например, вынести нормализацию в отдельный шаг перед постановкой в очередь) либо сериализовать в ключ только уже нормализованные поля. `backend/src/handlers/mod.rs`
...
- [ ] 🔴 **319.** (баг) edit.quality (CRF) нигде не валидируется и не клампится перед подстановкой в аргументы ffmpeg -> Добавить в normalize_edit_request кламп quality к диапазону, зависящему от кодека/формата (например 0..=63 для vp9/av1, 0..=51 для x264/x265). `backend/src/model.rs`
- [ ] 🔴 **353.** (баг) render_cache_key считается по EditRequest до normalize_edit_request, а не после нормализации -> Переместить вычисление cache_key после normalize_edit_request, либо клонировать/нормализовать req перед первым вызовом render_cache_key. `backend/src/handlers/mod.rs`
...
- [ ] 🟠 **320.** (проблема) edit.codec и edit.format — произвольные строки без whitelisting на границе домена -> Валидировать format/codec в normalize_edit_request через whitelist и возвращать 400 при неизвестном значении, либо перейти на serde enum с serde(other). `backend/src/model.rs`
...
- [ ] 🟠 **327.** (баг) crop с чётностью через 'w & !1' может дать w=0 при вырожденном прямоугольнике шириной 1 в args.rs, если вызвано в обход normalize_edit_request -> Добавить .max(2) прямо в video_filters рядом с '& !1', не полагаясь только на вызывающий код в handlers. `backend/src/tools/args.rs`
...
- [ ] 🟠 **348.** (проблема) quality: Option<u32> используется как CRF для принципиально разных кодеков (x264 0-51, vp9/av1 0-63) без указания в типе, что диапазоны отличаются -> Документировать диапазон по кодеку в doc-комментарии и/или клампить per-codec в normalize_edit_request, как указано в отдельном пункте про отсутствие валидации quality. `backend/src/model.rs`
...
- [ ] 🟡 **341.** (идея) Нет доменной проверки совместимости codec с format (например h265 запрошен вместе с format=webm) -> Возвращать явную ошибку или warning в normalize_edit_request, если codec задан вместе с форматом, где он не имеет смысла. `backend/src/tools/args.rs`
...
- [ ] 🟡 **344.** (дизайн) Добавление нового look-пресета требует знания имени в filter_preset, но нет единого списка допустимых имён, используемого и для валидации, и для UI -> Экспортировать список имён пресетов через отдельную fn filter_preset_names() -> &'static [&'static str] и переиспользовать её при валидации в normalize_edit_request. `backend/src/tools/args.rs`
...
- [ ] 🟡 **321.** (проблема) filter_preset принимает произвольную строку в edit.filter, неизвестное имя тихо игнорируется без ошибки -> Либо валидировать filter в normalize_edit_request по тому же списку имён, либо возвращать ошибку из filter_preset для явно нераспознанных значений. `backend/src/tools/args.rs`
...
- [ ] 🟡 **329.** (проблема) Комментарий 'the frontend clamps speed to that range' в audio_filters недостоверен — на самом деле клампит backend (normalize_edit_request), а не фронтенд -> Поправить комментарий на 'normalize_edit_request on the backend clamps speed to that range before this runs'. `backend/src/tools/args.rs`
- [ ] 🟡 **331.** (проблема) rotate: i32 в модели принимает произвольные значения, эффективно используются только четыре через rem_euclid(360) -> Валидировать rotate в normalize_edit_request через whitelist [0,90,180,270] с явной ошибкой на прочих значениях. `backend/src/model.rs`
...
- [ ] 🟡 **340.** (проблема) Crop.x/y/w/h — u32 без верхней доменной границы, полагается только на clamp_rect_to_source в handlers -> Задокументировать в model.rs, что Crop валиден только после normalize_edit_request, либо добавить конструктор/valid()-метод, чтобы инвариант был явным в типе. `backend/src/model.rs`
...
- [ ] 🟡 **349.** (проблема) censor_color: Option<String> — строка вместо enum, хотя допустимых значений всего четыре -> Либо валидировать значение в normalize_edit_request по тому же whitelist, что и sanitize_color, либо перейти на serde enum с #[serde(rename_all)]. `backend/src/model.rs`
...
- [ ] 🟡 **360.** (проблема) edit_handler читает req.format для output_ext ещё до normalize_edit_request, хотя format вообще не валидируется normalize_edit_request -> Явно задокументировать в normalize_edit_request или рядом с ней, какие поля EditRequest она не трогает (format/codec/quality/filter/censor_color), чтобы вызывающий код не предполагал обратное. `backend/src/handlers/mod.rs`
...
- [ ] 🟡 **401.** (улучшение) GET /api/projects возвращает полные video/edit JSON-блобы для каждого проекта в списке. -> Добавить облегчённый list-запрос без video/edit колонок для отображения списка проектов, подгружая полный JSON только по get_project. `backend/src/handlers/projects.rs`
...
- [ ] 🟡 **405.** (проблема) Project (только Serialize) заставляет хендлеры парсить входящий JSON вручную через serde_json::Value. -> Ввести отдельный ProjectUpsertRequest со строгими полями и Deserialize, заменив ручной разбор serde_json::Value в хендлере. `backend/src/handlers/projects.rs`
...
- [ ] 🔴 **410.** (проблема) SSRF-проверка URL и фактический HTTP-запрос yt-dlp разделены во времени (TOCTOU/DNS rebinding) -> Резолвить хост самостоятельно, передавать yt-dlp уже проверенный IP (--resolve или явный IP в URL с Host-заголовком) вместо доменного имени. `backend/src/tools/net.rs`
...
- [ ] 🟠 **425.** (проблема) probe_video как единственная валидация загруженного файла не проверяет соответствие реального кодека/контейнера расширению из sanitize_ext -> После probe_video сверять фактический контейнер/кодек с ожидаемым по расширению и отклонять явные несоответствия. `backend/src/handlers/mod.rs`
...
- [ ] 🟡 **415.** (дизайн) Комментарий 'basic SSRF guard' занижает фактический объём защиты в net.rs -> Расширить комментарий с перечислением конкретных классов угроз, которые покрывает validate_url, и явно указать, чего не покрывает (редиректы, TOCTOU). `backend/src/tools/net.rs`
...
- [ ] 🟡 **444.** (проблема) import_handler и upload_handler не ограничивают число одновременно принимаемых upload-запросов -> Ограничить одновременные upload_handler через отдельный semaphore или общий с jobs_semaphore лимит на конкурентные дисковые операции. `backend/src/handlers/mod.rs`
...
- [ ] 🟠 **471.** (проблема) normalize_edit_request совмещает валидацию и мутацию/клэмпинг в одной функции с 10+ независимыми проверками -> Разделить на validate_edit_request (только Err) и clamp_edit_request (только приведение к границам), вызываемые последовательно. `backend/src/handlers/mod.rs`
...
- [ ] 🟡 **489.** (баг) ensure_project_json_size сериализует JSON дважды на каждый upsert без необходимости в успешном пути -> Считать размер по одной комбинированной сериализации {video, edit} или переиспользовать уже посчитанные байты для последующей записи в БД вместо повторной сериализации. `backend/src/handlers/projects.rs`
...
- [ ] 🟡 **493.** (проблема) normalize_edit_request жёстко перечисляет пары (поле, русское сообщение) построчно вместо декларативного описания -> Осознанно оставить как есть для текущего размера EditRequest (27 полей управляемо) либо ввести макрос/массив описаний только если список продолжит расти. `backend/src/handlers/mod.rs`
...
- [ ] 🟠 **509.** (проблема) applySnapshot перезаписывает весь state.edit целиком через JSON.parse при undo/redo -> Использовать Object.assign(state.edit, JSON.parse(json)) с явным пересозданием вложенных объектов, чтобы сохранить единый реактивный корень. `frontend/src/store.ts`
...
- [ ] 🟡 **662.** (проблема) Нет property/fuzz-тестов для normalize_edit_request и clamp_rect_to_source -> Добавить proptest, генерирующий source_width/height и rect в диапазоне 0..=8000, и утверждающий, что clamp_rect_to_source никогда не паникует и всегда возвращает rect внутри границ источника. `backend/src/handlers/mod.rs`
- [ ] 🟡 **663.** (проблема) SSRF-тесты не покрывают DNS rebinding между валидацией URL и фактическим скачиванием -> Добавить unit-тест на уровне tools.rs с подменяемым resolver (trait-инъекция), который меняет ответ между двумя вызовами и проверяет, что download-путь тоже валидирует резолвнутый IP, а не только исходный hostname. `backend/tests/api.rs`
...
- [ ] 🟡 **667.** (проблема) sanitize_ext не покрыт ни одним прямым или HTTP-level тестом -> Добавить unit-тест sanitize_ext(Some("clip.MP4")), sanitize_ext(Some("noext")), sanitize_ext(Some("a.???")) и HTTP-тест upload с такими filename в multipart. `backend/src/handlers/mod.rs`
...
- [ ] 🟠 **706.** (проблема) /api/health не проверяет реальное состояние SQLite или диска, только статичные флаги ffmpeg/ytdlp -> Добавить в health_handler лёгкий SELECT 1 к Db и проверку доступности STORAGE_DIR на запись, включив их в ответ. `backend/src/handlers/health.rs`
...
- [ ] 🔴 **736.** (проблема) EditRequest — это одновременно wire DTO, доменная модель и вход билдера ffmpeg -> Развести на EditRequestDto (serde-граница), EditPlan/EditState (валидированная доменная модель) и FfmpegSpec (вход билдера), с явным преобразованием между ними. `backend/src/model.rs`
...
- [ ] 🔴 **742.** (баг) render_cache_key считается до normalize_edit_request, что делает кэш нестабильным для эквивалентных запросов -> Переместить вызов normalize_edit_request перед вычислением render_cache_key, до входа в spawn (после первого probe_video, если размеры нужны для клэмпа rect). `backend/src/handlers/mod.rs`
...
- [ ] 🟠 **741.** (проблема) normalize_edit_request мутирует EditRequest на месте, смешивая валидацию, клэмпинг и парсинг ошибок -> Разделить на чистую функцию validate(&EditRequest) -> Result<(), ValidationError> и normalize(&mut EditRequest) без сообщений об ошибках внутри валидации. `backend/src/handlers/mod.rs`
...
- [ ] 🟠 **745.** (проблема) Неизвестное значение filter молча игнорируется вместо ошибки валидации -> В normalize_edit_request проверять edit.filter по белому списку допустимых пресетов и возвращать anyhow::bail! при несовпадении. `backend/src/tools/args.rs`
...
- [ ] 🟠 **758.** (проблема) Trim/Crop/Scale не гарантируют инвариант end > start / w,h > 0 на уровне типа -> Ввести smart-constructor (TryFrom) для Trim/Crop/Scale, возвращающий Result при нарушении инварианта прямо на этапе десериализации. `backend/src/model.rs`
- [ ] 🟠 **760.** (проблема) codec='h265' проверяется строковым сравнением в двух независимых функциях -> Валидировать codec по белому списку ['h264','h265'] в normalize_edit_request и завести Codec enum вместо Option<String>. `backend/src/tools/args.rs`
...
- [ ] 🟠 **767.** (баг) reverse при segments переворачивает уже склеенный ролик целиком, а не порядок и содержимое каждого сегмента отдельно -> Задокументировать точную семантику reverse+segments в комментарии к video_filters либо запретить их комбинацию в normalize_edit_request. `backend/src/tools/args.rs`
- [ ] 🟠 **773.** (проблема) project_upsert_handler валидирует форму project JSON вручную через Value-индексацию вместо десериализации в типизированный DTO -> Определить `#[derive(Deserialize)] struct ProjectUpsertRequest { video_id: String, video: Value, edit: Value, name: Option<String> }` и заменить Json<Value> на Json<ProjectUpsertRequest>. `backend/src/handlers/projects.rs`
...
- [ ] 🟡 **751.** (идея) Нет OutputSpec, отделённого от EditRequest — формат/кодек/качество перемешаны с эффектами обработки -> Вынести format/codec/quality в отдельный OutputSpec, передаваемый вместе с EditRequest, но независимо валидируемый и кэшируемый. `backend/src/model.rs`
...
- [ ] 🟡 **772.** (проблема) output_ext и push_video_codec независимо перечисляют один и тот же список форматов -> Определить единый Format enum с методом .extension() и .video_codec_kind(), чтобы оба свойства выводились из одного описания формата. `backend/src/tools/args.rs`
- [ ] 🟡 **774.** (проблема) ensure_project_json_size сериализует video/edit ещё раз только для проверки размера, а upsert_project сериализует их снова -> Сериализовать video/edit один раз в handler, передать готовые строки в Db::upsert_project (изменив сигнатуру на &str) и проверять их len() напрямую. `backend/src/handlers/projects.rs`
...
- **Файлы:** `backend/src/tools/mod.rs` (`download_video` ~63-115), `backend/src/tools/net.rs`.
- **Шаги:**
